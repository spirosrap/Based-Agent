...
def run_autonomous_loop(agent, interval=30):
    client = Swarm()
    messages = []
    ...
    while True:
        ...
        if elapsed_time >= interval:
            # Generate a thought
            thought = (
                "As an autonomous blockchain agent, I should consider my next action. "
                "What interesting or useful task can I perform on the Base blockchain? "
                "I could create a token, transfer assets, deploy an NFT, or check balances. "
                "Let me think about what would be most beneficial or interesting right now."
            )
            messages.append({"role": "user", "content": thought})
            
            print(f"\n\033[90mAgent's Thought:\033[0m {thought}")
            
            # Run the agent to generate a response to the thought
            response = client.run(
                agent=agent,
                messages=messages,
                stream=True
            ...
            # Act on the thought
            action_thought = (
                "Now that I've considered my options, I should take action based on my thoughts. "
                "I'll execute the most appropriate blockchain interaction."
            )
            messages.append({"role": "user", "content": action_thought})
            
            print(f"\n\033[90mAgent's Action:\033[0m {action_thought}")
            
            # Run the agent to perform an action
            action_response = client.run(
                agent=agent,
                messages=messages,
                stream=True
            )
            
            # Process and print the streaming response for the action
            action_response_obj = process_and_print_streaming_response(action_response)
            
            # Update messages with the new response
            messages.extend(action_response_obj.messages)
